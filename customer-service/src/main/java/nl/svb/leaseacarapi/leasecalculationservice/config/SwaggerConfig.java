...
public class SwaggerConfig {
  ...
  private static final ApiInfo API_INFO = new ApiInfo("Api Documentation Customer Service",
      "Customer Service Api Documentation", "-", "-", DEFAULT_CONTACT, "-", "-", new ArrayList<>());

  /**
   * Creates and returns a {@link Docket} containing API documentation for the
   * {@link CustomerController} implementation.
   *
   * @return {@link Docket}
   */
  @Bean
  public Docket api() {
    ...
    return new Docket(DocumentationType.SWAGGER_2).apiInfo(API_INFO).select()
        .paths(PathSelectors.regex("/customer-service/customers.*")).build();
